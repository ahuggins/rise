defmodule RiseWeb.HomeLive do
  use RiseWeb, :live_view

  alias Rise.Posts
  alias Rise.Posts.Post
  ...
  def render(assigns) do
    ...
    <div id="feed" phx-update="stream" class="flex flex-col gap-2">
      ...
      >
        <.button class="bg-white" type="button" phx-click={show_modal("zoom-modal-#{dom_id}")}>
    ...
    <div :for={{dom_id, post} <- @streams.posts} id={"models-#{dom_id}"}>
      <.modal id={"zoom-modal-#{dom_id}"}>
        <img src={post.image_path} />
        <p><%= post.caption %></p>
      </.modal>
    </div>
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Rise.PubSub, "posts")
  ...
  def handle_event("save-post", %{"post" => post_params}, socket) do
    ...
    |> case do
      {:ok, post} ->
        ...
        Phoenix.PubSub.broadcast(Rise.PubSub, "posts", {:new, Map.put(post, :user, user)})
  ...
  defp consume_files(socket) do
    consume_uploaded_entries(socket, :image, fn %{path: path}, _entry ->
      dest = Path.join([:code.priv_dir(:rise), "static", "uploads", Path.basename(path)])
      File.cp!(path, dest)
